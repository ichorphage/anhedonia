	...
	if state then
		...
		env.funcs.box("machine teleport loop started")
			...
			while env.stuf.afe.running do
				if not tplooppause or env.stuf.actionqueuerunning or env.funcs.getgamestat("panicmode") or env.funcs.getgamestat("flooractive") or env.funcs.getstats("player", env.stuf.char).extracting then
					env.funcs.tomachine("tp")
				end
				t(3)
			end
